...
int complete_subcmdv(cmd_tbl_t *cmdtp, int count, int argc, char *const argv[],
...
#if defined(CONFIG_CMD_MEMORY) || defined(CONFIG_CMD_I2C) ||                   \
	defined(CONFIG_CMD_ITEST) || defined(CONFIG_CMD_PCI) ||                \
...
extern int do_poweroff(cmd_tbl_t *cmdtp, int flag, int argc,
		       char *const argv[]);
...
#define ROCK_SUBCMDS_RELOC(_cmdname)                                         \
...
#define ROCK_SUBCMDS_DO_CMD(_cmdname)                                        \
	...
	{                                                                      \
		...
		if (!subcmd || argc - 1 > subcmd->maxargs)                     \
			return CMD_RET_USAGE;                                  \
                                                                               \
		if (flag == CMD_FLAG_REPEAT && !cmd_is_repeatable(subcmd))     \
			return CMD_RET_SUCCESS;                                \
                                                                               \
		return subcmd->cmd_rep(subcmd, flag, argc - 1, argv + 1,       \
				       repeatable);                            \
	}

#ifdef CONFIG_AUTO_COMPLETE
#define ROCK_SUBCMDS_COMPLETE(_cmdname)                                      \
	static int complete_##_cmdname(int argc, char *const argv[],           \
				       char last_char, int maxv, char *cmdv[]) \
	{                                                                      \
		return complete_subcmdv(_cmdname##_subcmds,                    \
					ARRAY_SIZE(_cmdname##_subcmds),        \
					argc - 1, argv + 1, last_char, maxv,   \
					cmdv);                                 \
	}
#else
#define ROCK_SUBCMDS_COMPLETE(_cmdname)
...
#define ROCK_SUBCMDS(_cmdname, ...)                                          \
	static cmd_tbl_t _cmdname##_subcmds[] = { __VA_ARGS__ };               \
	ROCK_SUBCMDS_RELOC(_cmdname)                                         \
	ROCK_SUBCMDS_DO_CMD(_cmdname)                                        \
	ROCK_SUBCMDS_COMPLETE(_cmdname)

#ifdef CONFIG_CMDLINE
#define ROCK_CMDREP_MKENT_COMPLETE(_name, _maxargs, _cmd_rep, _usage, _help, \
				     _comp)                                    \
	{                                                                      \
#_name, _maxargs, _cmd_rep, cmd_discard_repeatable, _usage,    \
			_CMD_HELP(_help) _CMD_COMPLETE(_comp)                  \
	}

#define ROCK_CMD_MKENT_COMPLETE(_name, _maxargs, _rep, _cmd, _usage, _help,  \
				  _comp)                                       \
	{                                                                      \
#_name, _maxargs,                                              \
...
#define ROCK_CMD_COMPLETE(_name, _maxargs, _rep, _cmd, _usage, _help, _comp) \
	ll_entry_declare(cmd_tbl_t, _name, cmd) = ROCK_CMD_MKENT_COMPLETE(   \
		_name, _maxargs, _rep, _cmd, _usage, _help, _comp);

#define ROCK_CMDREP_COMPLETE(_name, _maxargs, _cmd_rep, _usage, _help,       \
			       _comp)                                          \
	ll_entry_declare(cmd_tbl_t, _name, cmd) =                              \
		ROCK_CMDREP_MKENT_COMPLETE(_name, _maxargs, _cmd_rep,        \
					     _usage, _help, _comp)

#else
#define ROCK_SUBCMD_START(name) static cmd_tbl_t name[] = {};
#define ROCK_SUBCMD_END
...
#define ROCK_CMD(_name, _maxargs, _rep, _cmd, _usage, _help)                 \
	ROCK_CMD_COMPLETE(_name, _maxargs, _rep, _cmd, _usage, _help, NULL)

#define ROCK_CMD_MKENT(_name, _maxargs, _rep, _cmd, _usage, _help)           \
	ROCK_CMD_MKENT_COMPLETE(_name, _maxargs, _rep, _cmd, _usage, _help,  \
				  NULL)

#define ROCK_SUBCMD_MKENT_COMPLETE(_name, _maxargs, _rep, _do_cmd, _comp)    \
	ROCK_CMD_MKENT_COMPLETE(_name, _maxargs, _rep, _do_cmd, "", "", _comp)

#define ROCK_SUBCMD_MKENT(_name, _maxargs, _rep, _do_cmd)                    \
	ROCK_SUBCMD_MKENT_COMPLETE(_name, _maxargs, _rep, _do_cmd, NULL)

#define ROCK_CMD_WITH_SUBCMDS(_name, _usage, _help, ...)                     \
	ROCK_SUBCMDS(_name, __VA_ARGS__)                                     \
	ROCK_CMDREP_COMPLETE(_name, CONFIG_SYS_MAXARGS, do_##_name, _usage,  \
