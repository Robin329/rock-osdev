...
static volatile u64 g_core0_ttbr0_el3 = 0;
static volatile u64 g_gd = 0;
...
void setup_pgtables(void)
{
	...
	for (i = 0; mem_map[i].size || mem_map[i].attrs; i++) {
		pr_info("\n [%s] virt[%d]:0x%llx size:0x%llx\n", __func__, i,  mem_map[i].virt,  mem_map[i].size);
		add_map(&mem_map[i]);
	}
}
...
void set_secure_instruction_fetch(void)
{
	u64 scr_el3;
	asm volatile("mrs %0, scr_el3\n": "=r"(scr_el3):: "memory");
	/* do not mask fiq irq for cpu interface */
	scr_el3 &= ~(SCR_EL3_SIF);
	asm volatile("msr scr_el3, %0": : "r"(scr_el3): "cc");
}
...
__weak void mmu_setup(void)
{
	...
	/* get core ttbr0 */
	g_core0_ttbr0_el3 = read_sysreg(ttbr0_el3);
	g_gd = (u64)gd;
}
...
u64 get_current_ttbr(void)
{
    ...
    if (el == 1) {
            ...
            asm volatile("mrs %0, ttbr0_el2" : "=r" (ttbr): : "memory");
    } else if (el == 3) {
            asm volatile("mrs %0, ttbr0_el3" : "=r" (ttbr): : "memory");
    } else {
            hang();
    }
    asm volatile("isb");
 ...
 * @parameter1: virtual address
 * @parameter2: see MT_*
 * @paremeter3: see MEMORY_*
 */
__attribute__((__unused__)) void change_memory_attr(u64 va, u32 memory_type, u32 is_non_secure)
{
	u64 *pte = (u64 *)(get_current_ttbr() & (~0xfff));
	int start_level = 1;
	u64 idx;
	int i;

	for (i = start_level; i < 4; i++) {
		idx = (va >> level2shift(i)) & 0x1FF;
		pte += idx;
		pr_info("idx=%llx PTE %p at level %d: %llx\n", idx, pte, i, *pte);

		/* PTE is table get next pte */
		if (pte_type(pte) == PTE_TYPE_TABLE) {
			pte = (u64 *)(*pte & (~0xfff));
		} else {
			if (memory_type != MT_IGNORE) {
				*pte = PTE_BLOCK_MEMTYPE(memory_type & 0x7) | (*pte & ~(0x7ULL << 2));
			}

			if (memory_type != MEMORY_NS_IGNORE) {
				*pte = ((is_non_secure & 0x1) << 5) | (*pte & ~(0x1ULL << 5));
			}
			pr_info("\n set attr [%p]:%llx\n", pte, *pte);
			break;
		}
...
void enable_other_cores_cache(void)
{
	u32 el;
	...
	set_secure_instruction_fetch();
	el = current_el();
	pr_debug("\n *******el:%d core0_ttbr0_el3:%llx\n", el, g_core0_ttbr0_el3);
	if (g_core0_ttbr0_el3 == 0) {
		pr_err("\n g_core0_ttbr0_el3 is not init \n");
		return;
	}
	set_ttbr_tcr_mair(el, g_core0_ttbr0_el3, get_tcr(el, NULL, NULL),
			  MEMORY_ATTRIBUTES);
	...
	asm volatile("mov x18, %0" : : "r" (g_gd) : "cc");
}

void enable_cache_at_el2_el1(void)
{
	u32 el;
