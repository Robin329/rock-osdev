...
void fixup_cmdtable(cmd_tbl_t *cmdtp, int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		cmdtp->cmd_rep = (int (*)(struct cmd_tbl_s *, int, int,
					  char *const[], int *))addr;

		addr = (ulong)(cmdtp->cmd) + gd->reloc_off;
#ifdef DEBUG_COMMANDS
		printf("Command \"%s\": 0x%08lx => 0x%08lx\n", cmdtp->name,
		       (ulong)(cmdtp->cmd), addr);
#endif
		cmdtp->cmd = (int (*)(struct cmd_tbl_s *, int, int,
				      char *const[]))addr;
		...
		if (cmdtp->complete) {
			...
			cmdtp->complete = (int (*)(int, char *const[], char,
						   int, char *[]))addr;
		}
#endif
		cmdtp++;
